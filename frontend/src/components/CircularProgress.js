function CircularProgress({ raised, goal, image, title, size = 220, strokeWidth = 7, onClick, imageCount }) {
  const pct = Math.min((raised / goal) * 100, 100);
  const radius = (size - strokeWidth) / 2;
  const circumference = 2 * Math.PI * radius;
  const offset = circumference * (1 - pct / 100);

  return (
    <div className="cd-progress-wrap" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="cd-ring">
        <circle
          cx={size / 2} cy={size / 2} r={radius}
          fill="none" stroke="#e0e0e0" strokeWidth={strokeWidth}
        />
        <circle
          cx={size / 2} cy={size / 2} r={radius}
          fill="none" stroke="#4caf50" strokeWidth={strokeWidth}
          strokeLinecap="round"
          strokeDasharray={circumference}
          strokeDashoffset={offset}
          transform={`rotate(-90 ${size / 2} ${size / 2})`}
        />
      </svg>
      <img
        src={image} alt={title} className="cd-ring-img"
        onClick={onClick}
        style={onClick ? { cursor: 'pointer' } : undefined}
      />
      {imageCount > 0 && (
        <div className="cd-image-badge" onClick={onClick} style={onClick ? { cursor: 'pointer' } : undefined}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
            <path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/>
          </svg>
          <span>{imageCount}</span>
        </div>
      )}
    </div>
  );
}

export default CircularProgress;
