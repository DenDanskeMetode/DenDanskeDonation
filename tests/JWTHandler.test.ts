...
describe('JWTHandler', () => {
  describe('issueToken', () => {
    it('returns a JWT string with three parts', () => {
      const token = issueToken({ userId: 1, email: 'a@b.com', username: 'foo' });
      expect(typeof token).toBe('string');
      expect(token.split('.')).toHaveLength(3);
    });

    it('encodes the payload correctly', () => {
      const payload = { userId: 42, email: 'user@test.com', username: 'testuser' };
  ...
  describe('validateToken', () => {
    it('returns the correct payload for a valid token', () => {
      const payload = { userId: 1, email: 'a@b.com', username: 'foo' };
      const token = issueToken(payload);
      const decoded = validateToken(token);
    ...
    it('throws on an expired token', () => {
      const token = issueToken({ userId: 1, email: 'a@b.com', username: 'foo' }, '-1s');
